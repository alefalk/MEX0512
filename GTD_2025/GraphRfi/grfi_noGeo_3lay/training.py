...
class GCNClassifier(torch.nn.Module):
    def __init__(self, in_channels, h1, h2, h3, num_classes, dropout=0.5):
        super().__init__()
    ...
    def forward(self, x, edge_index):
        ...
        x = self.conv4(x, edge_index)  # logits (no activation)
        return x, embeddings
...
def train_joint_gcn_nrf(
    ...
    gcn = GCNClassifier(in_channels=x_tv.size(1),
                       h1=nrf_cfg['h1'], h2=nrf_cfg['h2'], h3=nrf_cfg['h3'],
                       num_classes=num_classes,
                       dropout=nrf_cfg['dropout']).to(device)

    # Select feature extractor
    input_size = len(df_all.columns) - 1 + nrf_cfg['h3']
    feat_layer_cls = {
