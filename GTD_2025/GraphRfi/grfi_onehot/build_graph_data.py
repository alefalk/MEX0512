...
def make_infer_edge_index(edge_index, trainval_mask, test_mask):
    """
    Keep: Train/Val → Train/Val  and  Train/Val → Test.
    Drop everything starting from Test (so no Test→Train, no Test→Test).
    """
    src, dst = edge_index
    keep = (trainval_mask[src] & trainval_mask[dst]) | (trainval_mask[src] & test_mask[dst]) | (test_mask[src] & test_mask[dst]) | (test_mask[src] & trainval_mask[dst])
    return edge_index[:, keep]
