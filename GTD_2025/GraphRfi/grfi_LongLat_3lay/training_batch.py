...
class GCNClassifier(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, num_classes, dropout=0):
        super().__init__()
...
def train_joint_gcn_nrf(
    ...
    input_size = len(df_all.columns) - 1 + hidden_dim
    feat_layer_cls = {
    ...
    ################ BATCHING ########################
    X_tv_tab = torch.tensor(
    node_scaler.transform(df_all.loc[trainval_idx, node_feature_cols].fillna(0.0).to_numpy()),
    dtype=torch.float32, device=device
    )

    # Local indices for train/val in the subgraph order
    train_ids = torch.nonzero(train_mask_sub, as_tuple=False).view(-1)
    val_ids   = torch.nonzero(val_mask_sub, as_tuple=False).view(-1)

    # DataLoader that yields *indices* only (we’ll gather tensors per batch)
    train_loader = DataLoader(train_ids, batch_size=nrf_cfg['batch_size'], shuffle=True, drop_last=False)

    #####################################

    start_time = time.time()
    for epoch in range(nrf_cfg['epochs']):
        gcn.train(); neural_forest.train()
        optimizer.zero_grad()

        # 1) One full-batch GCN forward on the subgraph (keep computation graph!)
        logits_tv, emb_tv = gcn(x_tv, ei_tv)   # [M, C], [M, H]

        # 2) Full-batch GCN loss on train nodes
        loss_gcn = F.cross_entropy(logits_tv[train_mask_sub], y_tv_vec[train_mask_sub])

        # 3) NRF mini-batch loss accumulated over train_loader
        total_nrf_loss = 0.0
        n_seen = 0
        for idx in train_loader:                    # idx are local positions in the subgraph
            # Build NRF batch input: [tabular || GCN embedding]
            X_batch = torch.cat([X_tv_tab[idx], emb_tv[idx]], dim=1)   # keep emb_tv as part of graph
            prob_b  = neural_forest(X_batch)
            loss_b  = F.nll_loss(torch.log(prob_b.clamp_min(1e-8)), y_tv_vec[idx])
            # Accumulate *scaled* so magnitude doesn’t depend on batch size
            total_nrf_loss += loss_b * idx.numel()
            n_seen += idx.numel()

        total_nrf_loss = total_nrf_loss / max(n_seen, 1)

        # 4) Joint loss and single backward/step per epoch
        loss = loss_gcn + total_nrf_loss
        loss.backward()
        optimizer.step()
        ...
        with torch.no_grad():
            logits_eval, emb_eval = gcn(x_tv, ei_tv)
            X_val = torch.cat([X_tv_tab[val_ids], emb_eval[val_ids]], dim=1)
            prob_val = neural_forest(X_val)
            val_pred = prob_val.argmax(dim=1)
            val_true = y_tv_vec[val_ids]
            ...
            if val_acc > best_acc:
                ...
                #best_metrics = get_metrics(y_true_np, y_pred_np, y_proba_np, val_acc, None)
    ...
    if nrf_cfg.get("final_evaluation", True):
        ...
        ei_infer = make_infer_edge_index_strict(ei_full, tv_mask_full, test_mask_full)
        ...
        with torch.no_grad():
            ...
            best_metrics = get_metrics(y_test_true, y_test_pred, y_test_proba, test_acc, None)
            print(f"Best test acc: {test_acc:.4f} @ epoch {0}")
    else:
        print(f"Best validation acc: {best_acc:.4f} @ epoch {0}")
        test_acc = best_acc

    return test_acc, best_epoch, _, epoch_logs
