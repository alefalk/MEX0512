...
def build_graph_data(df, label_index, continuous_col, dist_threshold_km=1.0):
    ...
    def mark(df_split, mask, coord_to_index):
    ...
    # Mark splits
    mark(train_df, train_mask, coord_to_index)
    mark(val_df, val_mask, coord_to_index)
    mark(test_df, test_mask, coord_to_index)

    print("Training nodes count:", train_mask.sum())  # Expect 1142
    print("Validation nodes count:", val_mask.sum())  # Expect 600
    print("Test nodes count:", test_mask.sum())  # Expect 600
...
def train_joint(y_nrf, non_geo_features,
    ...
    for epoch in range(args['epochs']):
        ...
        input_features = torch.cat([non_geo_features[row_train_mask]], dim=1)

        out_forest = neural_forest(input_features)
        print(out_forest.shape)
        print(row_train_mask.shape)
        ...
        neural_forest.eval()
        with torch.no_grad():
            input_features = torch.cat([non_geo_features[row_val_mask]], dim=1)
    ...
    if args['final_evaluation']:
        ...
        with torch.no_grad():
            input_features = torch.cat([non_geo_features[row_test_mask]], dim=1)
