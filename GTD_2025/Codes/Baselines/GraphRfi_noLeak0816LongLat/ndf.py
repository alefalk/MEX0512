...
class GTD100FeatureLayer(nn.Sequential):
    def __init__(self, input_size=47, dropout_rate=0.0, shallow=True):
        super().__init__()
        self.shallow = shallow
        if shallow:
            self.add_module('linear', nn.Linear(input_size, 32))
...
class GTD200FeatureLayer(nn.Sequential):
    def __init__(self, input_size=47, dropout_rate=0.0, shallow=True):
        ...
        self.shallow = shallow
        if shallow:
            self.add_module('linear', nn.Linear(input_size, 1024))
            if dropout_rate and dropout_rate > 0:
                self.add_module('dropout', nn.Dropout(dropout_rate))
        else:
            raise NotImplementedError
...
class GTD478FeatureLayer(nn.Sequential):
    def __init__(self, input_size=47, dropout_rate=0.0, shallow=True):
        ...
        self.shallow = shallow
        if shallow:
            self.add_module('linear', nn.Linear(input_size, 768))
            if dropout_rate and dropout_rate > 0:
                self.add_module('dropout', nn.Dropout(dropout_rate))
        else:
            raise NotImplementedError
