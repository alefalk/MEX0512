...
def build_graph_data(df, label_index, continuous_col, dist_threshold_km=1.0):
    ...
    full_coords = df['encodedlonglat'].drop_duplicates().reset_index(drop=True)
    ...
    longlat_coords = torch.tensor(df['encodedlonglat'].apply(lambda x: list(x)).to_list(), dtype=torch.float32)
    non_location_cols = [col for col in df.columns if col not in ['encodedlonglat', 'gname']]
    ...
    row_to_node_index = torch.tensor(df['encodedlonglat'].map(coord_to_index).values, dtype=torch.long)
