...
class RF_nog:
    def __init__(self, trainpath, testpath):
        ...
        combined_numeric = pd.concat([train_features[numeric_cols], test_features[numeric_cols]])
        full_combined = pd.concat([combined_numeric, geo_onehot], axis = 1)

        train_features = full_combined.iloc[:len(train_features)]
        test_features = full_combined.iloc[len(train_features):]
