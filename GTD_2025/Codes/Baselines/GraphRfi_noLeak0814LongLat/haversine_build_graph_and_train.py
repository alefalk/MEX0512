...
def build_graph_data(df, label_index, continuous_col, dist_threshold_km=1.0):
    ...
    return full_coords, Data(x=x, edge_index=edge_index_train), edge_index_full, y_gcn, y_nrf, nrf_input, train_df, val_df, test_df, train_mask, val_mask, test_mask, row_to_node_index, index_to_label
...
def train_joint(df, full_coords, data, edge_index_full, y_gcn, y_nrf, non_geo_features, train_df, val_df, test_df,
    ...
    input_size = len(df.columns) + args["embed_dim"]
    feat_layer_cls = {
    ...
    row_train_mask = torch.tensor(df.index.isin(train_df.index), device=device)
    row_val_mask   = torch.tensor(df.index.isin(val_df.index), device=device)
    row_test_mask  = torch.tensor(df.index.isin(test_df.index), device=device)
    ...
    patience = 400 if args.get('final_evaluation', True) else 100
