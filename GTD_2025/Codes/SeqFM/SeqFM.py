...
class SeqFM(BaseModel):
    ...
    def forward(self, X):
        ...
        static_U = self.static_U(static_U_X)  # (N, emb_dim)
        static_U = static_U.unsqueeze(1)      # (N, 1, emb_dim)

        static_E = self.static_E(static_E_X)  # (N, S, emb_dim)

        if self.feature:
            feature_E = self.feature_E(self.feature)
            static_E = torch.cat((static_U, static_E, feature_E), dim=1)
        else:
            static_E = torch.cat((static_U, static_E), dim=1)
        ...
        dynamic_freq = 1 / lengths.float()
        dynamic_freq = dynamic_freq.view(-1, 1, 1)
