...
def _equal_edges(df_all: pd.DataFrame, equal_cols, device="cuda", max_full_clique_size: int = 1000):
    ...
    for _, g in df_all.groupby(equal_cols, sort=False):
        ...
        if n <= max_full_clique_size:
            # all unordered pairs i<j
            I, J = np.triu_indices(n, k=1)
        else:
            # very large group: connect a ring + kNN(1) inside group to avoid O(n^2)
            # ring
            I = np.arange(n); J = (np.arange(n) + 1) % n
            # plus each node to its next-2 neighbor (loose extra)
            I = np.concatenate([I, np.arange(n)])
            J = np.concatenate([J, (np.arange(n) + 2) % n])
...
def make_infer_edge_index_allow_test2test(edge_index, trainval_mask, test_mask):
    """
    Keep: Train/Val → Train/Val, Train/Val → Test, and Test → Test.
    Still blocks Test → Train/Val.
    """
    src, dst = edge_index
    keep = (
        (trainval_mask[src] & trainval_mask[dst]) |   # TV → TV
        (trainval_mask[src] & test_mask[dst])   |     # TV → Test
        (test_mask[src] & test_mask[dst])             # Test → Test (allowed)
    )
    return edge_index[:, keep]
...
def build_graph_on_index_set(
    ...
    if edge_mode in ("knn", "adaptive", "hybrid_equal_knn"):
        k_eff = min(k, max(1, M-1))
        nbrs_all = NearestNeighbors(n_neighbors=min(k_eff+1, max(2, M))).fit(edge_Z)
        dists, knn_idx = nbrs_all.kneighbors(edge_Z)
        if edge_mode in ("knn", "hybrid_equal_knn"):
            rows = np.repeat(np.arange(M), k_eff)
            cols = knn_idx[:, 1:1+k_eff].reshape(-1)
            ei_knn = torch.tensor([rows, cols], dtype=torch.long, device=device)
            edges.append(ei_knn)
        if edge_mode == "adaptive":
            rad_i = dists[:, k_eff]
            rows_list, cols_list = [], []
            k_check = min(3*k_eff+1, knn_idx.shape[1])
            for i in range(M):
                for jj in range(1, k_check):
                    j = knn_idx[i, jj]
                    if dists[i, jj] <= rad_i[i]:
                        rows_list.append(i); cols_list.append(j)
            ei_adapt = torch.tensor([rows_list, cols_list], dtype=torch.long, device=device)
            edges.append(ei_adapt)

    if edge_mode in ("radius", "hybrid_equal_radius"):
        if chosen_radius is None:
            # choose ε from TRAIN ONLY but measured in THIS subset space if subset contains train nodes;
            # in practice we pass the train idxs, so build ε from train∩subset
            train_in_sub = [rid for rid in (train_idx or []) if rid in pos]
            k_eff = min(k, max(1, len(train_in_sub)-1))
            if k_eff >= 1:
                Ztr = edge_Z[[pos[r] for r in train_in_sub]]
                nbrs_tr = NearestNeighbors(n_neighbors=k_eff+1).fit(Ztr)
                d_tr, _ = nbrs_tr.kneighbors(Ztr)
                chosen_radius = float(np.median(d_tr[:, k_eff]))
            else:
                chosen_radius = 0.0  # degenerate small set
        A = radius_neighbors_graph(edge_Z, radius=chosen_radius, mode='connectivity', include_self=False)
        r, c = A.nonzero()
        ei_rad = torch.tensor([r, c], dtype=torch.long, device=device)
        edges.append(ei_rad)

    if edge_mode in ("equal", "hybrid_equal_knn", "hybrid_equal_radius"):
        if not equal_cols:
            raise ValueError("equal_cols required for equality edges")
        ei_eq_df = _equal_edges(df_all.loc[sub_ids], equal_cols=equal_cols, device=device)
        if ei_eq_df.numel() > 0:
            # remap df indices (which here are sub_ids) to local 0..M-1
            r = torch.tensor([pos[int(i)] for i in ei_eq_df[0].tolist()], device=device)
            c = torch.tensor([pos[int(i)] for i in ei_eq_df[1].tolist()], device=device)
            ei_eq = torch.stack([r, c], dim=0)
            edges.append(ei_eq)
